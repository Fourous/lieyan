...
Page({
  ...
  onReady: function() {
    ...
    // console.log(that.data.tid);
    wx.request({
      url: 'https://www.lieyanwenhua.com/Getime',
      method: 'POST',
      data: {
        "tid":that.data.tid,
  ...
  yuyue: function(e){
    ...
    if (sta== 0){
      ...
      that.del(that.data.openid, that.data.tid, weekI + 1, classI + 1);
    }else{
      that.data.classsigns[weekI].classsigns[classI].num--;
      console.log(that.data.classsigns[weekI].classsigns[classI].num);
      that.setData({
      ...
      that.add(that.data.openid, that.data.tid, weekI + 1, classI + 1);
    }
    
  },
  add:function(openid,tid,weekid,classid){
    wx.request({
      url: 'https://www.lieyanwenhua.com/userappoint',
      method: 'POST',
      data: {
        "openid": openid,
        "tid": tid,
  ...
  del: function (openid, tid, weekid, classid) {
    wx.request({
      url: 'https://www.lieyanwenhua.com/userdeleappoint',
      method: 'POST',
      data: {
        "openid": openid,
        "tid": tid,
